...
int repTL_data = 2;
int spltM_data = 3;
//uint16_t registerValue = 0x0;

LPD8806 repTL = LPD8806(nLEDs, repTL_data, clockPin);
LPD8806 spltMtoRTR = LPD8806(nLEDs, spltM_data, clockPin);
LPD8806 repTR = LPD8806(nLEDs, repR_data, clockPin);
...
void setup()
{
  encoder = new ClickEncoder(33, 34, 35);
}

void loop()
{
  clockRot += encoder->getValue(); // write a value to clockRot use to delay.
  // need to test the outputs of the rot and make a function to map to a delay.
}

void out_spltM(int num)
{
  int i;
  for(i = 0; i < nLEDs; i++)
  {
    if(bitRead(num, i) == 1)
    {
      spltM.setPixelColor(i, spltM.Color(127, 0, 0));
    }
    else
    {
      spltM.setPixelColor(i, 0);
    }
  }
  spltM.show();
